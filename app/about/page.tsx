...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const descriptions: Record<string, string> = {
    pt: "Por que Verso existe e o que a publicação editorial da exímIA se propõe a fazer.",
    en: "Why Verso exists and what exímIA's editorial publication aims to do.",
    es: "Por qué existe Verso y qué se propone hacer la publicación editorial de exímIA.",
  };
...
export default async function AboutPage() {
  ...
  if (locale !== "pt") {
    // Load translated MDX from static files
    try {
      const filePath = path.join(process.cwd(), "content", "about", `${locale}.mdx`);
      const raw = await fs.readFile(filePath, "utf-8");
      content = await renderMDX(raw);
    } catch {
      // Fallback to PT inline content if file missing
      content = null;
    }
  }
